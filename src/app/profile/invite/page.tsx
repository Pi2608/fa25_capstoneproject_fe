    ...
    <div className="min-w-0 relative px-4 py-6">
      <div className="mx-auto max-w-4xl space-y-4">
        <div className="flex items-center justify-between gap-3">
          <div>
            <h1 className={`text-2xl sm:text-3xl font-semibold flex items-center gap-2 mb-6 ${isDark ? "text-white" : "text-gray-900"}`}>
              <span>
                {tr("title")}
              </span>
              ...
              <span
                className={[
                  ...
                  pendingCount > 0
                    ? (isDark ? "bg-emerald-500/15 text-emerald-100 border-emerald-400/40" : "bg-emerald-100 text-emerald-700 border-emerald-200")
          ...
          <button
            ...
            className={[
              ...
              loading
                ? (isDark ? "border-zinc-800 bg-zinc-900 text-zinc-500 cursor-wait" : "border-gray-200 bg-gray-100 text-gray-400 cursor-wait")
                : themeClasses.button,
            ].join(" ")}
          >
            {tr("refresh")}
        ...
        {toast && (
          <div className={`rounded-lg border px-3 py-2 text-sm shadow-sm ${isDark ? "border-emerald-400/40 bg-emerald-500/10 text-emerald-100" : "border-emerald-200 bg-emerald-50 text-emerald-800"}`}>
            {toast}
          </div>
        )}
        ...
        {loading && (
          <div className="space-y-3">
            {[...Array(3)].map((_, i) => (
              <div
                key={i}
                className={`h-20 rounded-xl border shadow-sm animate-pulse ${themeClasses.panel}`}
              />
            ))}
          </div>
        )}
        ...
        {!loading && err && (
          <div className={`rounded-xl border p-4 text-sm shadow-sm ${isDark ? "border-red-500/50 bg-red-500/10 text-red-100" : "border-red-200 bg-red-50 text-red-800"}`}>
            {err}
          </div>
        )}

        {!loading && !err && pending.length === 0 && (
          <div className={`rounded-xl border p-6 text-sm shadow-sm ${themeClasses.panel} ${themeClasses.textMuted}`}>
            {tr("empty_pending")}
          </div>
        )}
          ...
          <div className="space-y-3">
            {pending.map((inv) => (
              <div
                ...
                className={`rounded-xl border p-4 shadow-sm hover:shadow-md transition-shadow flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 ${themeClasses.panel}`}
              >
              ...
              <div className="space-y-2">
                {invitations
                  ...
                  .map((inv) => (
                    <div
                      key={inv.invitationId}
                      className={`rounded-lg border px-3 py-2 text-sm flex items-center justify-between shadow-sm ${themeClasses.panel} ${themeClasses.textMuted}`}
                    >
