...
function BankFormDialog({
  isOpen, isEditMode, saving, form, workspaces, selectedWorkspaceId,
  onChange, onWorkspaceChange, onSave, onClose,
}: {
  isOpen: boolean;
  isEditMode: boolean;
  saving: boolean;
    ...
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/40">
      <div className="w-full max-w-lg rounded-2xl bg-white p-5 shadow-xl dark:bg-zinc-900">
        <div className="mb-4 flex items-center justify-between">
          ...
          <button onClick={onClose} className="text-sm text-zinc-500 hover:text-zinc-800">✕</button>
        </div>

        <div className="space-y-4">
          <Field label="Tên bộ câu hỏi">
            <input name="bankName" value={form.bankName} onChange={onChange}
              className="w-full rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm text-zinc-900 outline-none focus:ring-2 focus:ring-emerald-500 dark:border-white/10 dark:bg-zinc-800 dark:text-zinc-50" placeholder="VD: Bộ câu hỏi chương 1..." />
          </Field>

          <Field label="Mô tả">
            <textarea name="description" value={form.description} onChange={onChange}
              className="w-full rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm text-zinc-900 outline-none focus:ring-2 focus:ring-emerald-500 dark:border-white/10 dark:bg-zinc-800 dark:text-zinc-50 min-h-[80px]" placeholder="Mô tả ngắn..." />
          </Field>

          <Field label="Danh mục">
            <input name="category" value={form.category} onChange={onChange}
              className="w-full rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm text-zinc-900 outline-none focus:ring-2 focus:ring-emerald-500 dark:border-white/10 dark:bg-zinc-800 dark:text-zinc-50" placeholder="VD: Lịch sử, Địa lý..." />
          </Field>

          <Field label="Tags (phân cách bằng dấu phẩy)">
            <input name="tags" value={form.tags} onChange={onChange}
              className="w-full rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm text-zinc-900 outline-none focus:ring-2 focus:ring-emerald-500 dark:border-white/10 dark:bg-zinc-800 dark:text-zinc-50" placeholder="VD: ôn tập, giữa kỳ" />
          </Field>
          ...
          <div className="flex items-center gap-4">
            <Checkbox name="isTemplate" checked={form.isTemplate} onChange={onChange} label="Dùng như template" />
            <Checkbox name="isPublic" checked={form.isPublic} onChange={onChange} label="Công khai trong tổ chức" />
          </div>
        </div>

        <div className="mt-5 flex justify-end gap-3">
          <button onClick={onClose} className="rounded-lg border border-zinc-200 px-3 py-1.5 text-sm text-zinc-700 hover:bg-zinc-50 dark:border-white/10 dark:text-zinc-100 dark:hover:bg-white/10">Hủy</button>
          <button onClick={onSave} disabled={saving} className="rounded-lg bg-emerald-500 px-4 py-2 text-sm font-semibold text-white hover:bg-emerald-600">
            {saving ? "Đang lưu..." : isEditMode ? "Lưu thay đổi" : "Tạo bộ câu hỏi"}
          </button>
        </div>
      </div>
    </div>
...
function AttachDialog({
  isOpen, bank, maps, selectedMapId, saving, getWorkspaceName, bankDetails,
  onMapChange, onAttach, onClose,
}: {
  isOpen: boolean;
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/40 px-4">
      <div className="w-full max-w-md rounded-2xl bg-white p-5 shadow-xl dark:bg-zinc-900">
        <div className="mb-4 flex items-center justify-between">
          <div>
            <p className="text-xs uppercase tracking-wide text-zinc-500">Gắn vào Storymap</p>
        ...
        <div className="space-y-4 text-sm">
          <div className="rounded-lg border border-zinc-200 bg-zinc-50/80 p-3 dark:border-white/10 dark:bg-white/5">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-zinc-500">{getWorkspaceName((bank as any).workspaceId)}</span>
            </div>
            <div className="mt-2 text-xs text-zinc-500">Số câu hỏi: {questionCount}</div>
          </div>
          ...
          {maps.length === 0 ? (
            ...
            <Field label="Chọn storymap">
              <select value={selectedMapId} onChange={e => onMapChange(e.target.value)} className="w-full rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm text-zinc-900 outline-none focus:ring-2 focus:ring-emerald-500 dark:border-white/10 dark:bg-zinc-800 dark:text-zinc-50">
        ...
        <div className="mt-6 flex justify-end gap-3">
          <button onClick={onClose} className="rounded-lg border border-zinc-200 px-3 py-1.5 text-sm text-zinc-700 hover:bg-zinc-50 dark:border-white/10 dark:text-zinc-100 dark:hover:bg-white/10">Hủy</button>
          <button onClick={onAttach} disabled={!selectedMapId || saving} className="rounded-lg bg-sky-600 px-4 py-1.5 text-sm font-semibold text-white hover:bg-sky-700 disabled:opacity-60">
...
function RowMenu({
  state, deletingBankId, onEdit, onAttach, onDelete, onClose,
}: {
  ...
  return createPortal(
    <div className="fixed inset-0 z-40" onClick={onClose}>
      <div
        ...
        role="menu"
      >
        <MenuButton onClick={() => { onEdit(state.bank); onClose(); }}>Sửa thông tin</MenuButton>
        <MenuButton onClick={() => { onAttach(state.bank); onClose(); }}>Thêm vào storymap</MenuButton>
        <MenuButton
          danger
...
const MenuButton = ({ children, danger, disabled, onClick }: {
  children: React.ReactNode; danger?: boolean; disabled?: boolean; onClick: () => void;
}) => (
  <button
    onClick={onClick}
    disabled={disabled}
...
export default function QuestionBanksPage() {
  ...
  const getWorkspaceName = (workspaceId?: string | null) => {
    ...
    return data.workspaces.find(w => w.workspaceId === workspaceId)?.workspaceName ?? workspaceId;
  };


  const handleSave = async () => {
    if (!bankForm.form.bankName.trim()) return showToast("error", "Vui lòng nhập tên bộ câu hỏi");
    if (!bankForm.selectedWorkspaceId) return showToast("error", "Vui lòng chọn workspace");
    ...
    try {
      const payload = {
        ...
        tags: bankForm.form.tags.split(",").map(s => s.trim()).filter(Boolean),
  ...
  if (data.err || !data.org) {
    return <div className="max-w-3xl px-4 text-red-600">{data.err ?? "Không tìm thấy tổ chức"}</div>;
  }
      ...
      <div className="mx-auto max-w-6xl text-zinc-900 dark:text-zinc-50">
        ...
        <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-semibold sm:text-3xl">Bộ câu hỏi</h1>
              <p className="text-sm text-zinc-600 dark:text-zinc-400">{data.org.orgName}</p>
            </div>
          </div>
          <button onClick={bankForm.openCreate} className="rounded-lg bg-emerald-500 px-4 py-2 text-sm font-semibold text-white hover:bg-emerald-600">Tạo bộ câu hỏi</button>
        </div>
          ...
          <div className="inline-flex rounded-full bg-zinc-100 p-1 text-xs dark:bg-zinc-900">
            {(["my", "public"] as const).map(tab => (
              <button
                ...
                className={`rounded-full px-4 py-1.5 font-medium ${activeTab === tab
                  ? "bg-white text-zinc-900 shadow-sm dark:bg-zinc-800 dark:text-zinc-50"
                  : "text-zinc-500 hover:text-zinc-800 dark:text-zinc-400"
                  }`}
              >
                {tab === "my" ? `Của tôi (${data.myBanks.length})` : `Công khai (${data.publicBanks.length})`}
              </button>
            ))}
          </div>
        </div>
        ...
        <section className="rounded-2xl border border-zinc-200 bg-white shadow-sm dark:border-white/10 dark:bg-white/5">
          <div className="flex items-center justify-between border-b border-zinc-100 px-4 py-3 text-sm font-semibold dark:border-white/10">
            <h2>{activeTab === "my" ? "Bộ câu hỏi của tôi" : "Bộ câu hỏi công khai"}</h2>
            <span className="text-xs text-zinc-500">{displayBanks.length} bộ</span>
          </div>

          {displayBanks.length === 0 ? (
            <div className="px-4 py-6 text-sm text-zinc-500">
              {activeTab === "my" ? 'Chưa có bộ câu hỏi nào.' : "Chưa có bộ câu hỏi công khai nào."}
            </div>
          ) : (
              ...
              <table className="min-w-full text-sm">
                <thead>
                  <tr className="bg-zinc-50/70 text-zinc-700 dark:bg-zinc-900/70 dark:text-zinc-200">
                    {["Tên bộ", "Mô tả", "Số câu hỏi", "Tags", "Workspace", "Hành động"].map(h => (
                      <th key={h} className={`px-4 py-2 font-semibold ${h === "Hành động" ? "text-right" : "text-left"}`}>
                        {h}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {displayBanks.map(bank => {
                    ...
                    const tags = extra?.tags?.length ? extra.tags : normalizeTags(bank.tags);

                    return (
                      <tr key={id} className="border-t border-zinc-100 dark:border-white/10">
                        <td className="px-4 py-2 font-medium">{bank.bankName}</td>
                        <td className="px-4 py-2 text-zinc-600 dark:text-zinc-400">{bank.description || "—"}</td>
                        <td className="px-4 py-2">
                          <span className={count === 0 ? "text-zinc-400" : "font-semibold text-emerald-600"}>
                            {labelFromCount(count)}
                          </span>
                        </td>
                        <td className="px-4 py-2 text-zinc-600">{tags.length ? tags.join(", ") : "—"}</td>
                        <td className="px-4 py-2 text-xs text-zinc-500">{bank.bankName}</td>
                        <td className="px-4 py-2">
                          <div className="flex flex-wrap justify-end gap-2">
                            {activeTab === "my" ? (
                              <>
                                <button onClick={() => goToEditQuestions(id)} className="inline-flex items-center rounded-full bg-emerald-500 px-3 py-1 text-xs font-semibold text-white hover:bg-emerald-600">
                                  Tạo / sửa
                                </button>
                                <button
                              ...
                              <button onClick={() => goToEditQuestions(id)} className="inline-flex items-center rounded-full bg-zinc-800 px-3 py-1 text-xs font-semibold text-zinc-50 hover:bg-zinc-700">
                                Xem / sử dụng
                              </button>
                            )}
                          </div>
